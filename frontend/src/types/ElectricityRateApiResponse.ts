/**
 * Electricity rate pricing information in euros (converted from micro-units)
 */
export type PriceInEuros = {
  market_price: number;
  total_price_tax_included: number;
  price_incl_handling_vat: number;
  price_tax_with_vat: number;
};

/**
 * Metadata object for electricity rates - flexible key-value pairs
 */
export type ElectricityRateMetadata = Record<
  string,
  string | number | boolean | null
>;

/**
 * Pricing profile types for electricity rates
 */
export type PricingProfile = "standard" | "peak" | "off-peak" | "dynamic";

/**
 * Supported currency codes
 */
export type Currency = "EUR";

/**
 * Complete electricity rate response from the API
 */
export type ElectricityRatesResponse = {
  /** ISO 8601 datetime string for period start */
  period_start: string;
  /** ISO 8601 datetime string for period end */
  period_end: string;
  /** Date string in YYYY-MM-DD format */
  rate_date: string;
  /** Time period string (e.g., "14:00-15:00") */
  period: string;
  /** Market price in micro-units (e.g., 202500 = €0.2025) */
  market_price: number;
  /** Total price including tax in micro-units */
  total_price_tax_included: number;
  /** Price including handling and VAT in micro-units */
  price_incl_handling_vat: number;
  /** Tax price with VAT in micro-units */
  price_tax_with_vat: number;
  /** Pricing profile category */
  pricing_profile: PricingProfile | null;
  /** Carbon footprint in grams */
  carbon_footprint_in_gram: number | null;
  /** Sustainability score (1-100) */
  sustainability_score: number | null;
  /** Currency code */
  currency: Currency;
  /** Additional metadata */
  metadata: ElectricityRateMetadata | null;
  /** Prices converted to euros for convenience */
  prices_in_euros: PriceInEuros;
}[];

/**
 * API response wrapper for multiple electricity rates
 */
export type ElectricityRatesApiResponse = {
  data: ElectricityRatesResponse;
};
